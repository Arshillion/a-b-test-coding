...
.site-nav--compress#AccessibleNav,.site-nav--compress#AccessibleNav > li:nth-child(1){
    display: flex;
    align-items: center;
}
...
const lgInterval = setInterval(() => {
	try {
		...
		if (
			navActions &&
			navActionsLast &&
			!navActionsLast.querySelector('.nav-message')
		) {
...
const tabInterval = setInterval(() => {
	try {
		...
		if (tabNav && tabCartIcon && !tabNav.querySelector('.nav-message')) {
...
const mobInterval = setInterval(() => {
	try {
		...
		if (
			mobileLinkIcon &&
			mobileLink &&
			!mobileLink.querySelector('.nav-message')
		) {
			mobileLink.classList.add('mob-area');
			mobileLink.insertAdjacentElement('afterbegin', mobileLinkIcon);
			mobileLink.insertAdjacentHTML('afterbegin', new_elm_html);
