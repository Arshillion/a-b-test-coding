...
.like-products .like-product img.product-img {
    width: 75px;
    height: 100px;
    object-fit: cover;
    border-radius: 4px;
}
.like-products .like-product img.top-img {
    width: 25px;
    position: absolute;
    left: 10px;
    top: 10px;
...
const setProduct = (product) => {
	const { link, imgSrc, title, price, topImgSrc } = product;
	return `
        <a class="like-product" href="${link}" target="_blank">
            ${
							topImgSrc
								? `<img class="top-img" src="${topImgSrc}" alt="discount"/>`
								: ''
						}
...
const uspInterval = setInterval(() => {
	try {
		...
		if (exist_elm && !document.querySelector('.cart-usp-elm')) {
...
const productInterval = setInterval(() => {
	try {
		const exist_elm = document.querySelector('.cart-usp-elm');
		const products = document.querySelectorAll('.swiper-wrapper .swiper-slide');
		const discount = document.querySelector(
			'.swiper-wrapper .swiper-slide .discount-collection-badge'
		);
		const check = discount ? true : timePassed > 4000;
		if (
			exist_elm &&
			products?.length > 0 &&
			...
			products.forEach((product) => {
				...
				if (!productAdded.includes(dataSwiperSliderIndex)) {
					...
					const topImgSrc =
						product
							.querySelector('.discount-collection-badge.top_right')
			...
			exist_elm.insertAdjacentHTML(
				'beforebegin',
				likedProductsElm(likedProducts)
			);
